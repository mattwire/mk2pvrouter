 ...
 *      December 2014
 *
 * December 2014, modified to Mk2_RFdatalog_4_MJW.ino (Matthew Wire)
 *    Adds defines for PIN_SAVING_HARDWARE, RF12_RADIO_HARDWARE, ONEWIRE_TEMP_HARDWARE.
 * So it can be configured to run with every version of hardware.
 * Normal/AF switch is only read at startup (if ONEWIRE_TEMP_HARDWARE not defined).
 */
...
// Dig Pin 3 is temp sensor if defined, or switch otherwise
//#define ONEWIRE_TEMP_HARDWARE
// RFM12B radio fitted?  Can't be there if not PIN_SAVING_HARDWARE
//#define RF12_RADIO_HARDWARE

#include <Arduino.h>
#ifdef ONEWIRE_TEMP_HARDWARE
#include <OneWire.h>
#endif // ONEWIRE_TEMP_HARDWARE
#ifdef RF12_RADIO_HARDWARE
#include <JeeLib.h>     // JeeLib is available at from: http://github.com/jcw/jeelib
#endif // RF12_RADIO_HARDWARE
...
const byte optionInputPin = 3; // <-- the "mode" port 
...
OneWire oneWire(optionInputPin);
int tempTimes100;
#endif // ONEWIRE_TEMP_HARDWARE
...
//
const float powerCal_grid = 0.0445;  
const float powerCal_diverted = 0.0440;  
...
{  
...
#ifndef ONEWIRE_TEMP_HARDWARE
  pinMode(optionInputPin, INPUT);
  digitalWrite(optionInputPin, HIGH); // enable the internal pullup resistor
  delay (100); // allow time to settle
  int pinState = digitalRead(optionInputPin);  // initial selection and
  ...
  Serial.println();
  Serial.println("-------------------------------------");
  Serial.println("Sketch ID:   Mk2_RFdatalog_4_MJW.ino");
  Serial.println();
...
//  
  ...
  if (datalogEventPending)   
  {
    ...
    // First datalog after startup has this set to 0, discard it because readings are inaccurate
    if (copyOf_lowestNoOfSampleSetsPerMainsCycle > 0)
    {
    ...
    {     
      ...
      if (sampleSetsDuringThisCycle == 5) // part way through the +ve half cycle
      {
        ...
        if (energyInBucket_long > upperEnergyThreshold_long) {
          // when above the upper threshold, always set the triac to "on"
